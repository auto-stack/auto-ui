...
fn run_file(input: &PathBuf, output: Option<&PathBuf>, check: bool) -> Result<()> {
    ...
    let rust_code = transpile_file(input, None)
        .map_err(|e| {
            // Create miette-compatible error with source code and location
            let transpile_error = TranspileError::with_extracted_offset(
                input,
                source_code.clone(),
                &e,
            );

            // Display error using miette's fancy reporter
            if let Some(named_source) = transpile_error.source_code_for_miette() {
                eprintln!("{:?}", Report::new(transpile_error).with_source_code(named_source));
            } else {
                eprintln!("Error: {}", transpile_error);
            }
